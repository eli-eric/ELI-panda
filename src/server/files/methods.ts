...
export const getPathInfo = (req: NextApiRequest) => {
  const [, , targetCategory, targetId, fileCategory, fileId] = (req.url ?? '').split('/')
  const prefix = `/${fileCategory}/${targetCategory}/${targetId}/`
  const id = fileId
