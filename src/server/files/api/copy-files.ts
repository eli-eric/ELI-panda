...
const copyFiles = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (sourceUid === destinationUid) {
        logger.error(composeDebugMessage(req, 'Source and destination UIDs are the same'))
        return res.status(400).json({ error: 'Source and destination UIDs must be different' })
    }
    ...
    try {
        ...
        await Promise.all(
            objects.map(async obj => {
                ...
                const destinationObjectName = path.posix.join(destinationPrefix, relativePath)

                // Add logging
                logger.debug(`Copying object from ${sourceObjectName} to ${destinationObjectName}`)
        ...
        logger.error(`Failed to copy files from ${sourceUid} to ${destinationUid}: ${errorMessage}`)

        // Send the response
        res.status(500).json({ error: 'Failed to copy files', details: errorMessage })
    }
}
