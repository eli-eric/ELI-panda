...
export const PublicationWosImportDialog = ({
    ...
    const toggleRememberAuthor = (sourceIndex: number, checked: boolean) => {
        setRememberedAuthors(current => {
            const next = new Set(current)
    ...
    const handleSubmit = async () => {
        ...
        try {
            await onSubmit({
                ...
                authors,
                researcherIdLinks: buildWosResearcherIdLinks(
                    preview.authors,
                    authorSelections,
                            ...
                            <div key={author.sourceIndex} className="rounded-md border p-3">
                                ...
                                {canRemember && (
                                    <div className="mt-3 flex items-center gap-2">
                                        <Checkbox
                                            id={`wos-remember-${author.sourceIndex}`}
                                            checked={rememberedAuthors.has(author.sourceIndex)}
                                            onCheckedChange={checked =>
                                                toggleRememberAuthor(
                                                    author.sourceIndex,
                                                    checked === true,
                                                )
                                            }
