...
export const DoiLookupField = () => {
    ...
    const { rememberResearcherId } = useRememberResearcherId()
    ...
    const applyPreview = async (
        ...
        const rememberResults = await Promise.allSettled(
            selection.researcherIdLinks.map(link => rememberResearcherId(link)),
        )
        const couldNotRemember = rememberResults.some(result => result.status === 'rejected')

        closeModal(modalId)
        toast.success(fm({ id: wosMessages.applied }))
        if (couldNotRemember) {
            toast.error(fm({ id: wosMessages.errors.rememberFailed }))
        }
    }
