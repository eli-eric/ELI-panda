...
const updateByPath =
  (fn: UpdateFn) => (node: SystemTreeItem, path: SystemTreeItem['uid'][]) => {
    const [current, ...rest] = path
    const [target, others] = node?.children?.reduce(
      (acc, scope) =>
        scope.uid === current ? [scope, acc[1]] : [acc[0], [...acc[1], scope]],
    ...
    return {
      ...
      children: [
        ...others,
        rest.length > 0 ? updateByPath(fn)(updated, rest) : updated,
      ],
    }
  }
