...
type Props = {
    handleSubmit: (data: ServiceLineFormType, reset: UseFormReset<ServiceLineFormType>) => void
}
...
export const ServiceLineV3Wizard = ({ handleSubmit }: Props) => {
    ...
    const [previousServiceType, setPreviousServiceType] = useState<string | undefined>()
    ...
    const [, setColumnFilters] = useFilters(TABLE_IDS.SERVICE_LINE_ITEMS_SELECT, false, false)
    ...
    const serviceTypeData = useMemo(() => {
        return serviceName && serviceUid ? { name: serviceName, uid: serviceUid } : undefined
    ...
    const validateStep1 = useCallback((formData: ServiceLineFormType) => {
        return Boolean(formData.name && formData.serviceType && formData.price && formData.currency)
    }, [])
    ...
    const validateSelectedSystems = useCallback(() => {
        ...
        const rowSelection = instances[TABLE_IDS.SERVICE_LINE_ITEMS_SELECT]?.rowSelection || {}
