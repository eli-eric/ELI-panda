import { HomeIcon } from '@heroicons/react/20/solid'
import Link from 'next/link'
import { useRouter } from 'next/router'

import { useCataloguePath } from '@/hooks/usePath'
import { useCategoryEdit } from '@/modules/catalogue/hooks/useCategoryEdit'
import { PATH } from '@/types/constants/paths'

interface Props {
  navigationList: JSX.Element[] | undefined
  testId: string
}
// TODO: need refactor
const BreadcrumbListComponent = ({ navigationList, testId }: Props) => {
  const router = useRouter()
  const catalogueParentPath = useCataloguePath()
  const { getAddButton } = useCategoryEdit({ catalogueParentPath })
  const { search } = router.query
  const path = PATH.CATALOGUE + (search ? `?search=${search}` : '')
  return (
    <div data-testid={testId} id="breadcrump" className="bg-white pt-3 pb-3 ">
      <nav className="flex" aria-label="Breadcrumb">
        <ol role="list" className="flex space-x-4 bg-white px-6  ">
          <li className="flex">
            <div className="flex items-center">
              <Link data-testid={testId + '-home'} href={path} className="text-gray-400 hover:text-gray-500">
                <HomeIcon className="h-5 w-5 flex-shrink-0" aria-hidden="true" />
                <span className="sr-only">Home</span>
              </Link>
            </div>
          </li>
          {navigationList}
          {getAddButton()}
        </ol>
      </nav>
    </div>
  )
}
export default BreadcrumbListComponent
