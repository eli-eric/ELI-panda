...
const CategoryItemActions = ({ uid }: { uid: string }) => {
    ...
    const handleDelete = e => {
        e.stopPropagation()
        withWarningModal(mutate, fm({ id: message.catalogue.category.confirmDelete }))({})
    }
...
export const CategoryItemComponent = ({ category, setCategoryFilter }: Props) => {
    ...
    return (
        <Card
            ...
            onClick={e => {
                if (!(e.target instanceof HTMLElement) || !['BUTTON'].includes(e.target.tagName)) {
            ...
            <div className="flex items-center justify-between w-full min-w-0 gap-2">
                <div className="flex items-center gap-2 min-w-0 flex-1 justify-center">
                    <Avatar className="h-8 w-8">
                        <AvatarImage src={image || FALLBACK_IMAGE.url} alt={category.name} />
