...
export const CatalogueFilterSheet = ({
    ...
    useEffect(() => {
        if (
            catalogueCategoryProperties?.filter(prop => customFieldIdToSync.has(prop.property.uid))
                .length === 0
        ) {
            startTransition(() => {
                toggleDeleteCustom()
    ...
    return (
        <Form className="flex flex-col h-full justify-between" formMethods={filterFormMethods}>
            <CatalogueFilterForm
                tableId={tableId}
