    ...
    const newRows = originalData.map(row => {
        if (row.uid === destinationSystemUid) {
            return {
                ...row,
                hasSubsystems: true,
                subSystems: row.subSystems ? [...row.subSystems, ...movingSystems] : undefined,
            }
...
export const useMoveSubmit = ({
    ...
    const { destinationSystemsTableId, movingSystemsTableId } = useSystemsMoveStore()
    const { query: destinationTableQuery } = useQueryManager(destinationSystemsTableId)
    const { query: movingTableQuery } = useQueryManager(movingSystemsTableId)

    const queryClient = useQueryClient()

    const queryKeySubsystems: QueryFetcherKey = ['subsystems', { uid: destinationSystemUid }]
