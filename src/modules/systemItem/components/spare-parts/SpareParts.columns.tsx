...
export const useSparePartsColumns = () => {
    const columns = useMemo((): ColumnDef<SystemInterfaceSparePartsRelationship, string>[] => {
        const columns: ColumnDef<SystemInterfaceSparePartsRelationship, string>[] = [
            ...
            },
            {
                ...
                accessorFn: row =>
                    row.node.location?.name
                        ? row.node.location?.name + ' - ' + (row.node.location?.code || '')
                        : '',
            },
            {
                id: 'coverage',
            ...
            },
            {
                id: 'partNumber',
                header: 'Part Number',
                accessorFn: row => row?.node.physicalItem?.catalogueItem?.catalogueNumber as string,
            },
            {
                id: 'eun',
                header: 'EUN',
