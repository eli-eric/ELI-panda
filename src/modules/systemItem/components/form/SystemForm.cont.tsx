...
export const SystemForm: FC<PropsWithChildren> = ({ children }) => {
    ...
    const { updateSystem, loading } = useSystemUpdate(systemImageRef, physicalItem?.uid)
        ...
        defaultValues: {
            ...
            location: systemDetail?.location
                ? {
                      ...
                      name:
                          systemDetail?.location?.name + ' (' + systemDetail?.location?.code + ')',
        ...
        <Form className="relative" formMethods={formMethods} enableLeaveWarning={true}>
            ...
            <div className="container mx-auto max-w-7xl px-4 space-y-6">
                <CardUI>
                    <CardContent>
                        <Breadcrumbs
                            parentPath={(systemDetail?.parentPath as CodebookType[]) || parentPath}
                        />
                    </CardContent>
                </CardUI>
                <CardUI
                    className={cn('shadow-lg', getBorderBySystemLevel(systemLevel as SystemLevel))}
                >
                    <CardContent>
