...
import { CATALOGUE_ITEM_HISTORY_QUERY_KEY } from '../../types/constants'

export interface CatalogueHistoryEntry {
    at: string
    action: string
    changes: FieldChangeEntry[]
    user: { uid: string; fullName?: string | null } | null
}

interface CatalogueItemHistoryResponse {
...
const query = gql`
    query CatalogueItemHistoryInExplorer($where: CatalogueItemWhere) {
...
` as TypedDocumentNode<CatalogueItemHistoryResponse, Record<string, unknown>>
...
export const useCatalogueItemHistory = (uid: string | null) => {
    const { data, isLoading, error, refetch } = useGraphQL(query, {
        variables: { where: { uid } },
        customQueryKey: [CATALOGUE_ITEM_HISTORY_QUERY_KEY, uid],
        enabled: !!uid,
        staleTime: 30 * 1000,
    ...
    const entries: CatalogueHistoryEntry[] =
        data?.catalogueItems?.[0]?.updatedByConnection?.edges?.map(e => ({
            at: e.at,
            action: e.action,
