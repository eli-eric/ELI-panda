    ...
    useEffect(() => {
        if (response && uniqueRelationships) {
            const defaultRelationships = uniqueRelationships?.reduce(
                (acc, relationship) => {
                    if (relationship === 'WAS_UPDATED_BY' || relationship === 'WAS_MOVED_FROM') {
            ...
            const filteredNodes = response.nodes.filter(node =>
                filteredLinks.some(link => link.source === node.uid || link.target === node.uid),
            )
    ...
    const renderFilter = ({ open }: { open: boolean }) => {
        ...
        return (
            <Form formMethods={formMethods} className="col-span-4">
                <NodeFilters uid={uid} setData={setData} relationships={uniqueRelationships} />
            </Form>
        )
    }
