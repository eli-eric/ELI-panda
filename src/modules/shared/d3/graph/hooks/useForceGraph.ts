...
export const useForceGraph = ({
    ...
    const nodeSelectionRef = useRef<d3.Selection<
        ...
        unknown
    > | null>(null)
    const linkSelectionRef = useRef<d3.Selection<SVGPathElement, any, SVGGElement, unknown> | null>(
        null,
    )
    ...
    const dragstarted = (
        ...
        select(event.sourceEvent.currentTarget).attr('stroke', 'orange').attr('stroke-width', 3)

        d.fx = d.x
        d.fy = d.y
    }

    const dragged = (event: D3DragEvent<SVGCircleElement, GraphNode, unknown>, d: GraphNode) => {
        d.fx = event.x
        d.fy = event.y
    }

    const dragended = (event: D3DragEvent<SVGCircleElement, GraphNode, unknown>, d: GraphNode) => {
    ...
    useEffect(() => {
        ...
        const simulation = initializeForceSimulation(nodes, links, width, height, ticked)
        ...
        const node = addGraphNodes(g, nodes, circleRadius, dragstarted, dragged, dragended)
        nodeSelectionRef.current = node

        // Add node labels
        const nodeLabels = addNodeLabels(g, nodes, circleRadius, setSelectedNode, node)
        ...
        function ticked() {
            ...
            link.attr('d', d => {
                ...
                const curvature = totalLinks > 1 ? (linkIndex - (totalLinks - 1) / 2) * 30 : 0

                return `M${d.source?.x},${d.source?.y} Q${
                    (d.source?.x ?? 0) + dx / 2 + curvature
                },${(d.source?.y ?? 0) + dy / 2 + curvature} ${d.target?.x},${d.target?.y}`
            })
            ...
            // Update node labels
            nodeLabels.attr('x', d => d.x ?? 0).attr('y', d => (d.y ?? 0) - circleRadius - 5)
