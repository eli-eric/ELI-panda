...
export function addGraphLinks(g: Selection<SVGGElement, unknown, null, undefined>, links: any[]) {
    const link = g
...
export function addGraphNodes(
    ...
    nodes: GraphNode[],
    circleRadius: number,
    dragstarted: (event: D3DragEvent<SVGCircleElement, GraphNode, unknown>, d: GraphNode) => void,
    dragged: (event: D3DragEvent<SVGCircleElement, GraphNode, unknown>, d: GraphNode) => void,
    dragended: (event: D3DragEvent<SVGCircleElement, GraphNode, unknown>, d: GraphNode) => void,
) {
    const node = g
...
export function addNodeLabels(
    ...
    nodeLabels.on('click', function (event, d) {
        ...
        select(`circle[data-uid='${d.uid}']`).attr('stroke', 'orange').attr('stroke-width', 3)
    })
...
export function addRelationshipLabels(
    ...
    relationshipLabels.on('click', function (event, d) {
        link.attr('stroke', '#ccc').attr('stroke-width', 2).attr('marker-end', 'url(#arrowhead)')
        relationshipLabels.style('fill', '#666')

        // Highlight corresponding link
        link.filter(linkData => linkData === d)
            .attr('stroke', 'orange')
            .attr('stroke-width', 2)
