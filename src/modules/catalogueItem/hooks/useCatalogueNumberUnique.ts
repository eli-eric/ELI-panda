...
export const useCatalogueNumberUnique = ({
    ...
    const queryKey: QueryFetcherKey = ['catalogueNumberUniqueCheck', { query: { catalogueNumber } }]

    const { data, isFetching: isChecking } = useQuery({
        queryKey,
        queryFn: queryFetcher<CatalogueNumberUniqueResponse>('catalogueNumberUniqueCheck'),
