...
export const OrdersFilter = () => {
  ...
  const { setFilter } = useFormFilterState({ tableId: 'orders', enableQueryUrl: true })
  ...
  return (
    <div className={classNames('md:grid md:grid-cols-2 md:gap-4 md:min-w-[500px]')}>
      <div className="flex flex-col gap-2">
        ...
        <Input {...fields.orderNumber} onChange={setFilter(fields.orderNumber.name)} isFilter={true} />
        <FilterCheckboxes
          label="Order Status"
      ...
      <div className="flex flex-col gap-2">
        <Input {...fields.requestNumber} onChange={setFilter(fields.requestNumber.name)} isFilter={true} />
        <Input {...fields.contractNumber} onChange={setFilter(fields.contractNumber.name)} isFilter={true} />
        <Combobox {...fields.supplier} onChange={setFilter(fields.supplier.name)} isFilter={true} />
        <Listbox
          {...fields.procurementResponsible}
          onChange={setFilter(fields.procurementResponsible.name)}
          isFilter={true}
        />
        <Combobox {...fields.requestor} onChange={setFilter(fields.requestor.name)} isFilter={true} />
        {/* <FilterCheckboxes
      ...
      <Input className="col-span-2" {...fields.notes} onChange={setFilter(fields.notes.name)} isFilter={true} />
    </div>
  )
}
