...
import { useMutation, useQuery, type UseQueryResult, useSuspenseQuery } from '@tanstack/react-query'
...
export function useGraphQL<TResult, TVariables extends Variables>(
    document: TypedDocumentNode<TResult, TVariables>,
    options?: Omit<UseQueryOptions<TResult, Error, TResult, any[]>, 'queryKey' | 'queryFn'> & {
        variables?: TVariables
    ...
    const adjustedOptions: UseQueryOptions<TResult, Error, TResult, any[]> = {
        ...options,
        queryFn: async () => {
            return typedGraphQLRequest<TResult, TVariables>(document, options?.variables)
        },
...
export function useSuspenseGraphQL<TResult, TVariables extends Variables>(
    ...
    const adjustedOptions: UseSuspenseQueryOptions<TResult, Error, TResult, any[]> = {
        ...options,
        queryFn: async () => {
            return typedGraphQLRequest<TResult, TVariables>(document, options?.variables)
        },
...
export function useGraphQLMutation<TResult, TVariables extends Record<string, any> | undefined>(
    document: TypedDocumentNode<TResult, TVariables>,
    options?:
