...
const makePropertyEdge = (uid: string, name: string, value: string, groupUid?: string, groupName?: string, typeName?: string) => ({
  value,
  node: {
    uid,
    name,
...
describe('useItemPropertiesData', () => {
  ...
  it('marks overridden properties from service items', () => {
    ...
    const serviceItems = [
      makeServiceItem([
        makePropertyEdge('p1', 'Width', '150', 'g1', 'Dimensions'),
      ]),
    ]
