 ...
 */
export function TableFooter<T extends object>({ table, footerClassName }: TableFooterProps<T>) {
    // Don't render the footer if no footers defined in columns or no data
    const hasFooters = table.getAllColumns().some(column => !!column.columnDef.footer)
    ...
    return (
        <tfoot
            className={cn('bg-muted/50 border-t border-border', 'relative z-20', footerClassName)}
        >
            {table.getFooterGroups().map(footerGroup => (
                <tr key={footerGroup.id} className="border-t border-border">
                    {footerGroup.headers.map((header, headerIndex) => {
                        ...
                        const width = header.column.getSize() ? header.column.getSize() : undefined
