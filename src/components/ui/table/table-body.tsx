    ...
    return (
        <tbody>
            {rows.map((row, rowIndex) => {
                const customRowProps = getRowProps ? getRowProps(row.original, rowIndex) : {}

                // Determine row background color
                const defaultBgClass = rowIndex % 2 === 0 ? 'bg-background' : 'bg-muted/50'
                ...
                return (
                    ...
                    >
                        {row.getVisibleCells().map((cell, cellIndex) => {
                            // Get width from column definition if available
                            const width = cell.column.getSize() ? cell.column.getSize() : undefined
