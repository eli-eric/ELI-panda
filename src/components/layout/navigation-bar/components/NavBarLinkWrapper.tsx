import classNames from 'classnames'
import { usePathname } from 'next/navigation'

import type { NavBarLinkType } from '@/types/constants/paths'

interface NavBarLinkWrapperProps {
  children: React.ReactNode
  links?: NavBarLinkType[]
  className?: string
  open?: boolean
}
export const NavBarLinkWrapper = ({ children, links, className, open }: NavBarLinkWrapperProps) => {
  const pathName = usePathname()

  return (
    <div
      className={classNames(
        open
          ? 'block w-full text-left border-l-4 py-2 pl-3 pr-4 text-base '
          : 'inline-flex items-center border-b-2 px-1 pt-1 text-sm',
        'font-medium h-full',
        links?.some(link => link.path === pathName) ||
          pathName === links?.[0]?.path ||
          pathName.startsWith(links?.[0]?.path || '')
          ? 'text-gray-900 border-primary-500 dark:text-primary-600'
          : 'text-gray-500 hover:border-gray-300 hover:text-gray-700 dark:text-gray-200 border-transparent dark:hover:text-white dark:hover:border-primary-400',
        className
      )}
    >
      {children}
    </div>
  )
}
