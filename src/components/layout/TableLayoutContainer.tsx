...
export const TableLayoutContainer = ({ children, deps, className }: Props) => {
  ...
  useIsomorphicLayoutEffect(
    () => {
      const handleResize = () => {
        const searchBar =
          document.getElementById('search-bar')?.clientHeight || 0
        const tableHeading =
          document.getElementById('table-heading')?.clientHeight || 0
        const pageHead = document.getElementById('page-head')?.clientHeight || 0
        const emptyResults =
          document.getElementById('empty-results')?.clientHeight || 0
        const columnHiding =
          document.getElementById('column-hiding')?.clientHeight || 0
        const categoryList =
          document.getElementById('category-list')?.clientHeight || 0
        ...
        const cataloguePaging =
          document.getElementById('paging')?.clientHeight || 0
        const height =
          searchBar +
          tableHeading +
          pageHead +
